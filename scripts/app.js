...
const checkFirstVisit = () => {
  ...
  if (JSON.parse(localStorage.getItem('linkListfirstVisit')) === false) {
    return;;
  // If first visit, update localStorage
  } else if (!localStorage.getItem('linkListfirstVisit')) {
...
const renderLinkList = (page) => {
  const firstVisit = JSON.parse(localStorage.getItem('linkListfirstVisit'));
...
const checkValidURL = (form) => {
  ...
  // Check for valid URL using regex
...
const onCreateLinkSubmit = (e) => {
  ...
  localStorage.setItem('linkListData', JSON.stringify(localStorageData));
